...
export const ZoteroPlugin: Plugin = async () => ({
  tool: {
    zotero_count: tool({
      description: withPluginVersion("Use when you only need the total number of items in the Zotero library."),
      args: {},
      async execute() {
    ...
    zotero_stats: tool({
      description: withPluginVersion("Use when you need aggregate library statistics such as summary, item types, years, tags, or attachment counts."),
      args: {
    ...
    zotero_search: tool({
      description: withPluginVersion("Use when you need to search or filter the local Zotero library."),
      args: {
    ...
    zotero_get_item: tool({
      description: withPluginVersion("Use when you need the full Zotero record for a specific item key."),
      args: {
    ...
    zotero_children: tool({
      description: withPluginVersion("Use when you need attachments or notes for a Zotero item."),
      args: {
    ...
    zotero_update_item: tool({
      description: withPluginVersion("Use when you need to update fields on an existing Zotero item."),
      args: {
        item_key: tool.schema.string().describe("Zotero item key"),
        fields: tool.schema.record(tool.schema.string(), tool.schema.any()).describe("Fields to update"),
      },
      async execute(args) {
        return callZotero("update_item_fields", { item_key: args.item_key, fields: args.fields });
      },
    }),
    zotero_tags: tool({
      description: withPluginVersion("Use when you need to inspect or edit Zotero item tags."),
      args: {
        action: tool.schema.string().describe("'list', 'add', or 'remove'"),
        item_key: tool.schema.string().optional().describe("Item key for add/remove operations"),
        tags: tool.schema.array(tool.schema.string()).optional().describe("Tags to add or remove"),
      },
      async execute(args) {
    ...
    zotero_import: tool({
      description: withPluginVersion("Use when you need to import a DOI, ISBN, or PMID into Zotero."),
      args: {
    ...
    zotero_batch_add: tool({
      description: withPluginVersion("Use when you need to import many identifiers into Zotero at once."),
      args: {
    ...
    zotero_export: tool({
      description: withPluginVersion("Use when you need a text export of the Zotero library."),
      args: {
        action: tool.schema.string().describe("'json', 'bibtex', 'csv', 'ris', or 'csljson'"),
        collection: tool.schema.string().optional().describe("Optional collection key filter"),
      },
      async execute(args) {
    ...
    zotero_collections: tool({
      description: withPluginVersion("Use when you need to inspect collections or move an item into one."),
      args: {
    ...
    zotero_trash_items: tool({
      description: withPluginVersion("Use when you need to move one or more Zotero items to trash."),
      args: {
    ...
    zotero_check_pdfs: tool({
      description: withPluginVersion("Use when you need a summary of which Zotero items are missing PDFs."),
      args: {
        collection: tool.schema.string().optional().describe("Optional collection key filter"),
      },
      async execute(args) {
    ...
    zotero_crossref: tool({
      description: withPluginVersion("Use when you need to match citation text against the local Zotero library."),
      args: {
    ...
    zotero_find_dois: tool({
      description: withPluginVersion("Use when you need CrossRef-powered DOI backfilling for local Zotero items."),
      args: {
        apply: tool.schema.boolean().optional().describe("Write matched DOIs back to Zotero"),
    ...
    zotero_fetch_pdfs: tool({
      description: withPluginVersion("Use when you need to discover or attach open-access PDFs for Zotero items."),
      args: {
        dry_run: tool.schema.boolean().optional().describe("Show matches without downloading"),
        limit: tool.schema.number().optional().describe("Maximum items to process"),
        collection: tool.schema.string().optional().describe("Collection key filter"),
        download_dir: tool.schema.string().optional().describe("Directory to save PDFs into"),
        upload: tool.schema.boolean().optional().describe("Upload fetched PDFs to Zotero storage"),
        sources: tool.schema.array(tool.schema.string()).optional().describe("PDF sources to try in order"),
      },
      async execute(args) {
