...
MANAGER_REPO = "git+file:///home/dzack/opencode-plugins/zotero-manager"

def _run_dispatch(tool_name: str, args: dict) -> Any:
    cmd = [
        "uvx",
        "--from", MANAGER_REPO,
        "python", "-m", "zotero_librarian._dispatch",
        tool_name,
        json.dumps(args)
    ]
...
def zotero_batch_add(
    ...
    return _run_dispatch("batch_add_identifiers", {
...
def zotero_export(
    ...
    if collection is not None:
        args["collection_key" if action in ["ris", "csljson"] else "collection"] = collection
...
def zotero_find_dois(
    ...
    return _run_dispatch("find_missing_dois", {"apply": apply, "limit": limit, "collection": collection})
...
def zotero_fetch_pdfs(
    ...
    return _run_dispatch("fetch_pdfs", {
        "dry_run": dry_run,
        "limit": limit,
        "collection": collection,
...
def zotero_rename_pdf_attachments(
    ...
    return _run_dispatch("rename_pdf_attachments", {"dry_run": dry_run, "collection_key": collection_key})
