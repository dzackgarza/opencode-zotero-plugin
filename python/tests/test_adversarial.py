...
from zotero_librarian.client import get_zotero, count_items
from zotero_librarian.connector import local_write, save_item
from zotero_librarian.query import get_item, get_notes, get_attachments, get_citations, all_tags
...
def _collection_key(result: dict[str, Any]) -> str:
    key = result.get("details", {}).get("collection_key")
    assert key, result
    return key


def _note_key(result: dict[str, Any]) -> str:
    key = result.get("note_key")
    assert key, result
    return key
...
def test_item(zot):
    ...
    result = save_item(
        ...
        operation="test_adversarial_setup_item",
    )
    if not result.get("success"):
        pytest.skip(f"Failed to create test item: {result}")
...
class TestInvalidInputsNonExistentKeys:
    ...
    def test_update_nonexistent_item(self, zot):
        ...
        result = update_item_fields(zot, "NONEXISTENT123", {"title": "Test"})
        assert not result["success"], "Update nonexistent item should fail"
    ...
    def test_add_tag_to_nonexistent_item(self, zot):
        ...
        result = add_tags_to_item(zot, "NONEXISTENT123", ["tag"])
        assert not result["success"], "Add tag to nonexistent item should fail"

    def test_remove_tag_from_nonexistent_item(self, zot):
        """Remove tag from non-existent item."""
        result = remove_tags_from_item(zot, "NONEXISTENT123", ["tag"])
        assert not result["success"], "Remove tag from nonexistent item should fail"

    def test_move_nonexistent_to_collection(self, zot, temp_collection):
        """Move non-existent item to collection."""
        result = move_item_to_collection(zot, "NONEXISTENT123", temp_collection)
        assert not result["success"], "Move nonexistent item should fail"
...
class TestStateCorruption:
    ...
    def test_item_in_nonexistent_collection(self, zot, test_item, restore_item_state):
        ...
        result = move_item_to_collection(zot, test_item, "NONEXISTENT123")
        assert not result["success"], "Move into nonexistent collection should fail"
    ...
    def test_remove_from_nonexistent_collection(self, zot, test_item, restore_item_state):
        ...
        result = remove_item_from_collection(zot, test_item, "NONEXISTENT123")
        assert not result["success"], "Remove from nonexistent collection should fail"
    ...
    def test_add_to_multiple_collections(self, zot, test_item, restore_item_state):
        ...
        if coll1_result["success"] and coll2_result["success"]:
            coll1_key = _collection_key(coll1_result)
            coll2_key = _collection_key(coll2_result)
            add_item_to_collection(zot, test_item, coll1_key)
            add_item_to_collection(zot, test_item, coll2_key)
    ...
    def test_circular_collection_reference_attempt(self, zot, test_item):
        ...
        coll3_result = None
        try:
            coll2_result = create_collection(zot, f"Child - {time.time()}", parent_key=coll1_key)
            # Now try to make coll1 a child of coll2 (circular)
            # This should fail or be handled by Zotero
            coll2_key = _collection_key(coll2_result)
            coll3_result = create_collection(zot, f"Grandchild - {time.time()}", parent_key=coll2_key)
        except Exception:
...
class TestConcurrentModification:
    ...
    def test_read_modify_race(self, zot, test_item, restore_item_state):
        ...
        # Now apply a new update after the stale read was taken
        result = update_item_fields(zot, test_item, {"title": "Based on stale read"})
...
class TestTagOperations:
    ...
    def test_rename_nonexistent_tag(self, zot, test_item, restore_item_state):
        ...
        assert result.get("updated_items", 0) == 0, "Rename nonexistent tag should update 0 items"
    ...
    def test_delete_nonexistent_tag(self, zot, test_item, restore_item_state):
        ...
        assert result.get("updated_items", 0) == 0, "Delete nonexistent tag should update 0 items"
    ...
    def test_very_large_number_of_collections(self, zot, test_item, restore_item_state):
        ...
        try:
            for i in range(20):
                ...
                add_item_to_collection(zot, test_item, coll_key)
